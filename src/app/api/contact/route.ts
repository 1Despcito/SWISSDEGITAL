...
export async function POST(req: Request) {
  ...
  // Send notification + (for full leads) an autoresponder. Email failures must
  // not lose the lead — we log and still return ok so the UI can confirm.
  try {
    ...
    if (!notified) {
      // Email not configured (e.g. local dev) — log so the lead is recoverable.
      console.info('[lead:no-email-configured]', JSON.stringify(lead));
    }
  } catch (err) {
    console.error('[lead:send-failed]', err, JSON.stringify(lead));
    // Still acknowledge — a retry/backup pipeline can pick this up from logs.
  }
